...
# create : 생성 , table : 표 , test1 : 아무거나 표이름  
...
create table board(
    ...
    writer varchar(30)		# 게시물의 작성자	# private String wirter
);
...
create table waiting(
    ...
    count tinyint		# 인원수		# 20억		# private byte count 
);
select * from waiting;
...
create table test4(
    ...
    constraint primary key( 필드명4 )	-- 'pk필드명5' 속성을 PK 필드로 설정
);
select * from test4;
# [7]
create table test5(
...
create table member(						-- [1] 회원테이블
    ...
    mpwd varchar(30) not null ,				-- 회원비밀번호 : 문자최대30 , 공백x , 중복o
    mname varchar(10) not null ,			-- 회원닉네임 : 문자최대10 , 공백x , 중복o
    constraint primary key( mno )			-- 위에서 선언된 '회원번호'속성을 PK필드로 설정
);
select * from member;						-- 확인차 회원테이블 조회
create table board(							-- [2] 게시물 테이블
	bno int auto_increment ,				-- 게시물번호 : 정수 , 자동번호 , PK사용할예정
    btitle varchar(100) not null ,			-- 게시물제목 : 문자최대100 , 공백x , 중복o
