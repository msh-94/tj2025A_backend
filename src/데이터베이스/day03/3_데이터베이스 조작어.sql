...
# [1] INSERT : 레코드 삽입 , *레코드란? 테이블내 행(가로) 단위 의 데이터들 , 1줄 vs *1레코드* vs 1행
...
# (3) default 제약조건의 속성은 값 생략할경우 정의된 기본값 할당
...
# (5) usique 제약조건의 속성은 중복값일 경우 삽입 불가능 
...
select * from test; # 확인차 조회
 ...
 # [3] UPDATE : 레코드 수정 
 # mysql workbench 에서는 수정/삭제르 safe mode 이므로 불가능
 ...
 # 2. update 테이블명 set 속성명 = 새로운값 where 조건절;			: 특정한 레코드의 속성값 수정
 update test set mcount = 10 where mname = "유재석";
 # 3. update 테이블명 set 속성명1 = 새로운값1 , 속성명2 = 새로운값2;	: 다수의 속성값 수정
 update test set mcount = 20 , mname = "강호동2" where mno = 2;
 
 # [4] DELETE : 레코드 삭제
 # 2. delete from 테이블명 where 조건절		: 충족하는 레코드 삭제
