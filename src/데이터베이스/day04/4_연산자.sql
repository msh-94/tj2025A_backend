...
select mid as 회원아이디 , mname 회원명 from member;		-- as는 생략하고 띄어쓰기로 구분 가능하다
select mid as 회원아이디 from member as m;				-- member 테이블명을 'm' 으로 별칭 하다.
select mid 회원아이디 from member m;					-- as는 생략 가능하다.( 띄어쓰기 로 구분 )

# [2] deitinct , 속성값이 중복 인 레코드 제거
...
select * from member where mheight <= 162;						-- member테이블의 mheight속성의 값이 162 이하이면
select * from member where mheight >= 165 and mheight <= 170;	-- mheight속성의 값이 165이상 이면서 170이하
select * from member where mheight between 165 and 170;			-- mheight속성의 값이 165이상 이면서 170이하
...
select * from member where mname like "%이%";	-- 속성의 값에 "이"가 포함된				, x이x x이xx xx이x ~
select * from member where mname like "_이_";	-- 속성의 값이 두번재 글자가 "이"인 세글자		, x이x
# 문자패턴 : 속성명 like "문자패턴" , % : 문자대응(개수상관x) , _ : 문자대응(_수만큼대응)
...
select * from member order by mdebut;	-- asc/desc 생략시 자동으로 asc(기본값)
...
select * from member order by mheight desc limit 3;	-- mheight속성을 내림차순 으로 정렬 후 상위 3개 	
