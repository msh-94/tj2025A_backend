...
select customer , sum(order_qty) from orders where customer in( '김민준' , '이서연') group by customer having sum(order_qty) >= 4;
...
select genre , sum(stock) from books group by genre having genre = '소설';
...
select customer , count(book_id) from orders group by customer;
