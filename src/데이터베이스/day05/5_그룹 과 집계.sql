...
select bpname , bpirce from buy group by bpname; 
-- 오류 : bpname 속성값이 동일하더라도 bprice 다를수 있기 때문에 , 
...
select min( bamount ) from buy;		# min( 속성명 ) : 최소값
select max( bamount ) from buy;		# max( 속성명 ) : 최대값
...
select mid , bamount from buy group by mid;				-- 실행 불가능
select mid , sum( bamount ) from buy;					-- 실행 불가능
...
select bamount as 수량 from buy where 수량 > 3; -- 실행 불가 , where 에서 별칭x
# 2. 회원아이디 별 총구매금액 이 1000 초과 인 레코드 조회
...
select mid , sum( bamount * bprice ) as 총구매금액 from buy 
	group by mid having 총구매금액 >= 1000;	# 실행가능 : 별칭 이용한
select mid , sum( bamount * bprice ) as 총구매금액 from buy where 총구매금액 >= 1000
	group by mid; -- 실행 불가
select count(*) from buy where count(*) > 3;	# 실행불가능 : where 에서는 집계/그룹x having써야됨
