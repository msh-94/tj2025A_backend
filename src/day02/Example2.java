...
public class Example2 { // class start
    public static void main(String[] args) {//main start
        ...
        // 2. 종류
        //          .next() : 키보드로부터 입력받은 문자열 반환 , 단] 띄어쓰기 불가능
                    // String 변수명 = scan.next();
        //          .nextLine() : 입력받은 문자열(String) 반환 , 단] 띄어쓰기 가능
        //          * 주의할점 : nextLine() 앞에 다른 next()가 존재하면 enter(기행) 를 하나로 포함 하므로 문제발생
        ...
        String s1 = scan.next();    // ==  new Scanner( System.in).next();
        System.out.println("1. next result : " + s1);

        // 2. nextLine()
        System.out.print("2. nextLine() : ");
        scan.nextLine(); // 무의미한 nextLine() , nextLine() 문제점 해결
        String s2 = scan.nextLine();
        System.out.println("2. nextLine() result : "+ s2);
        ...
        // 10. scan.next().charAt( 0 ); : 입력받은 문자열 중에 첫번째 글자1개(char) 반환 , * nextChar() 없다.
