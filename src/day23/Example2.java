...
public class Example2 {// class start
    public static void main(String[] args) { // main start
        /*
            [ 컬렉션 프레임워크2 ]
                List 인터페이스  : 배열/링크 구조 , 인덱스 있다 중복 허용한다(인덱스가식별지)
                Set 인터페이스   : 집합구조  , 인덱스 없다 중복 허용하지않는다(자료가식별지) + 중복없는 집합
        */
        // [1] HashSet 객체 생성
        ...
        System.out.println(set1); // [유재석, 신동엽]
            // 5) .isEmpty() : 집합 내 자료가 하나도 없으면 true , 1개이상이면 false 반환
        ...
        for (String str : set1){ System.out.println(str); } // for end
        // 3) forEach 문 : 리스트/집합명.forEach( (반복변수명) -> { 실행문 } );
        set1.forEach((str) -> { System.out.println(str); }); // forEach end
